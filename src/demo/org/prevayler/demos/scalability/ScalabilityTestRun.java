...
/**
 * Represents a single run of a scalability test. To understand the
...
abstract class ScalabilityTestRun<C> {
    ...
    protected ScalabilityTestRun(ScalabilityTestSubject<C> subject, int numberOfObjects, int minThreads, int maxThreads) {
        ...
        if (minThreads < 1)
            throw new IllegalArgumentException("The minimum number of threads cannot be smaller than one.");
    ...
    /**
     * @return The name of the test to be executed. Example: "Prevayler Query
     *         Test".
     */
    ...
    /**
     * @return The number of operations the test managed to execute per second
