...
public class StubbornNetworkReceiverTest extends TestCase {
    ...
    /**
     * 
     * public void testClosing() throws Exception { ncor.close();
     * provider.checkClosed(); Thread.yield(); provider.printRules(); Thread []
     * threads = new Thread[Thread.activeCount()]; int threadCt =
     * Thread.enumerate(threads); for (int i=0; i<threadCt; i++) { if
     * (threads[i].getName().equals(threadName)) {
