 ...
 * 
 * Completes SessionId exchange in order to reconnect to the correct session.
 * Informs the outbound channel of network reconnection.
 * 
 * Forwards inbound data direct to the client. If an error occurs, closes the
 * session and notifies the outbound channel (StubbornNetworkProxy).
 * 
 * 
 */
...
public class StubbornServerReceiverImpl implements ObjectReceiver, StubbornNetworkClientConnector {
    ...
    /**
     * Provider has called in with a connection, need to get it going. First, I
     * wait for their session id. If it matches an existing session, I hook them
     * back together, if it is not found (ugh) or if it is zero (new session no
     * history), I send them a new id.
     */
