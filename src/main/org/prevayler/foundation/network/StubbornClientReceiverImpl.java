...
/**
 * The StubbronClientReceiver is an ObjectReceiver to the Client and has a
 * connection to the Socket (Provider).
 * 
 * It uses its being in the middle of the exchange to implement a recovery
 * mechanism on session failure.
 * 
 * At startup the Replication Client, sends its current session identifier to
 * the Replication Server, which if it matches a prior session, it reconnects
 * the session. It sends a new session identifier.
 * 
